<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-xl text-surface-900 leading-tight">Edit Ruangan</h2>
    </x-slot>
    ...
    <nav class="flex items-center gap-2 text-sm text-surface-700/50 mb-6">
        ...
        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
        <span class="text-surface-900 font-medium">Edit Ruangan</span>
    </nav>

    <!-- Page Title -->
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-surface-900">Edit Ruangan: {{ $room->name }}</h1>
        <p class="text-sm text-surface-700/50 mt-1">Perbarui informasi ruangan di bawah ini.</p>
    </div>
    ...
    <form action="{{ route('rooms.update', $room) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-surface-200/60 p-6 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            <label for="name" class="block text-sm font-medium text-surface-700 mb-2">Nama Ruangan</label>
                        ...
                        <!-- Kode Ruangan -->
                        <div>
                            <label for="code" class="block text-sm font-medium text-surface-700 mb-2">Kode Ruangan</label>
                        ...
                        <div>
                            <label for="capacity" class="block text-sm font-medium text-surface-700 mb-2">Kapasitas Maksimal (Orang)</label>
                            <div class="relative">
                                <input type="number" name="capacity" id="capacity" value="{{ old('capacity', $room->capacity) }}" min="1" required
                                       class="input-modern pr-14" placeholder="0">
                                <span class="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-surface-700/40 font-medium">pax</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-surface-200/60 p-6 shadow-sm">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            @php
                                $facilityOptions = ['Proyektor', 'AC', 'Sound System', 'Whiteboard', 'WiFi', 'Video Conference'];
                                $currentFacilities = old('facilities', $room->facilities ?? '');
                            @endphp
                            @foreach($facilityOptions as $facility)
                            <label class="flex items-center gap-2.5 p-3 rounded-xl border border-surface-200 hover:border-primary-300 hover:bg-primary-50/50 transition-all cursor-pointer group">
                                <input type="checkbox" name="facility_items[]" value="{{ $facility }}"
                                       ...
                                       {{ str_contains($currentFacilities, $facility) ? 'checked' : '' }}>
                                <span class="text-sm text-surface-700 group-hover:text-primary-700">{{ $facility }}</span>
                            </label>
                            @endforeach
                        </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-surface-200/60 p-6 shadow-sm">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center justify-between p-3 rounded-xl border border-surface-200 hover:border-primary-300 cursor-pointer transition-all has-[:checked]:border-primary-500 has-[:checked]:bg-primary-50/50">
                            <div class="flex items-center gap-3">
                                <span class="w-3 h-3 rounded-full bg-emerald-500"></span>
                                <span class="text-sm font-medium text-surface-700">Aktif</span>
                            </div>
                            <input type="radio" name="status" value="available" class="w-5 h-5 text-primary-600 border-surface-200 focus:ring-primary-500/40" {{ old('status', $room->status) === 'available' ? 'checked' : '' }}>
                        </label>
                        ...
                        <label class="flex items-center justify-between p-3 rounded-xl border border-surface-200 hover:border-primary-300 cursor-pointer transition-all has-[:checked]:border-primary-500 has-[:checked]:bg-primary-50/50">
                            ...
                            <input type="radio" name="status" value="maintenance" class="w-5 h-5 text-primary-600 border-surface-200 focus:ring-primary-500/40" {{ old('status', $room->status) === 'maintenance' ? 'checked' : '' }}>
                        </label>
                        ...
                        <label class="flex items-center justify-between p-3 rounded-xl border border-surface-200 hover:border-primary-300 cursor-pointer transition-all has-[:checked]:border-primary-500 has-[:checked]:bg-primary-50/50">
                            ...
                            <input type="radio" name="status" value="unavailable" class="w-5 h-5 text-primary-600 border-surface-200 focus:ring-primary-500/40" {{ old('status', $room->status) === 'unavailable' ? 'checked' : '' }}>
                        </label>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-surface-200/60 p-6 shadow-sm">
                    ...
                    <p class="text-xs text-surface-700/50 mb-5">Biarkan kosong jika tidak ingin mengubah.</p>

                    <div x-data="imageGalleryEdit()" class="space-y-4">
                        <!-- Current Photos -->
                        @if($room->photos)
                        <div class="mb-4">
                            <p class="text-sm font-medium text-surface-700 mb-2">Foto Saat Ini (Maks 3 total):</p>
                            <div class="grid grid-cols-3 gap-3">
                                @foreach($room->photos as $index => $photo)
                                <div class="relative group aspect-[4/3] rounded-lg overflow-hidden border border-surface-200" id="existing-photo-{{ $index }}">
                                    ...
                                    <button type="button" onclick="
                                        ...
                                        // Update count in alpine
                                        Alpine.store('editGalleryData').decrementExisting();
                                    " class="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-red-500/80 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600">
                        ...
                        <div x-show="canAddMore">
                            <label class="block w-full cursor-pointer">
                                <div class="border-2 border-dashed border-surface-200 rounded-xl p-6 text-center hover:border-primary-400 hover:bg-primary-50/30 transition-all">
                                    ...
                                    <p class="text-xs text-surface-700/40 mt-1">PNG, JPG up to 2MB</p>
                                </div>
                        ...
                        <script>
                            document.addEventListener('alpine:init', () => {
                                Alpine.store('editGalleryData', {
                                    existingCount: {{ $room->photos ? count($room->photos) : 0 }},
                                    decrementExisting() {
                                        this.existingCount = Math.max(0, this.existingCount - 1);
                                    }
                                });

                                Alpine.data('imageGalleryEdit', () => ({
                                    previews: [],
                                    files: [],
                                    get canAddMore() {
                                        return (Alpine.store('editGalleryData').existingCount + this.previews.length) < 3;
                                    },
                                    handleFiles(event) {
                                        const allowedToAdd = 3 - Alpine.store('editGalleryData').existingCount;
                                        if(allowedToAdd <= 0) return;

                                        const selectedFiles = Array.from(event.target.files).slice(0, allowedToAdd);
                                        this.previews = [];
        ...
        <div class="flex items-center justify-end gap-4 mt-8 pt-6 border-t border-surface-200/60">
            <a href="{{ route('rooms.index') }}" class="px-6 py-2.5 text-sm font-medium text-surface-700 hover:text-surface-900 transition-colors">
                Batal
            </a>
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-primary-600 text-white text-sm font-semibold rounded-xl hover:bg-primary-700 transition-colors shadow-sm shadow-primary-600/20"
                    onclick="
